...
def G0(step, name, time):
    ...
    train_x = np.reshape(data[name][8*time:8*(time+1),:,0],(-1,1024)).astype(np.float32)   
    train_ref = np.reshape(data[name][8*time:8*(time+1),:,1],(-1,1024)).astype(np.float32)
    ...
    weights_G = np.load('./results/'+name+'_weights/weight_G1_'+str(time)+'.npy', allow_pickle=True)
    weights_D = np.load('./results/'+name+'_weights/weight_D1_'+str(time)+'.npy', allow_pickle=True)
    ...
    np.save('./results/'+name+'_weights/weight_G0_'+str(time)+'.npy', weights_G)
    np.save('./results/'+name+'_weights/weight_D0_'+str(time)+'.npy', weights_D)
    return d_loss1, g_loss1