...
def G5(step, name, time):
    ...
    train_x = np.reshape(data[name][8*time:8*(time+1),:,5],(-1,1024)).astype(np.float32)   
    train_ref = np.zeros((8,1024)).astype(np.float32)
    # training sample
    ...
    np.save('./results/'+name+'_weights/weight_G5_'+str(time)+'.npy', weights_G)
    np.save('./results/'+name+'_weights/weight_D5_'+str(time)+'.npy', weights_D)
    return d_loss1, g_loss1